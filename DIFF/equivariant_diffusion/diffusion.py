...
class Diffusion(nn.Module):
    ...
    @staticmethod
    def gaussion_KL(q_mu_minus_p_mu_squared, q_sigma, p_sigma, d):
        """
        Computes the KL distance between two normal distribution.
        Args:
        q_mu_minus_p_mu_squaredL Squared difference between mean of 
        distribution q and distribution p: ||mu_q - mu_p||^2
        q_sigma: Standard deviation of distribution q
        p_sigma: Standart deviation of distribution p
        d: dimension
        Returns: KL Distance
        """
    ...
    def normalize(self, ligand = None, pocket = None):
        if ligand is not None:
            ...
            ligand['one_hot']/float() - self.norm_biases[1] / self.norm_values[1]
        
        if pocket is not None:
            pocket['x'] = pocket['x'] / self.norm_values[0]
            pocket['one_hot'] = (pocket['one_hot'].float() - self.norm_biases[1]) / self.norm_values[1]
    ...
    def subspace_dimensionality(self, input_size):
        ...
        where distribution on x are defined
        """
    ...
    @staticmethod
    def sample_center_gravity_zero_gaussian_batch(size, lig_indicies, pocket_indicies):

        assert len(size) == 2
        x = torch.randn(size, device=lig_indicies.device)

        x_projected = Diffusion.remove_mean_batch( x, torch.cat((lig_indicies, pocket_indicies)))
    ...
    def forward(self, ligand, pocket, return_info = False):
        ...
        # Normalize t to [0,1]
        # Step of s will bever be usedm since then p(x|z0) is computed
        s = s_int / self.T
        t = t_int / self.T

        # compute gamma_s and gamma t via network
        gamma_s = self.inflate_batch_array(self.gamma(s), ligand['x'])
        gamma_t = self.inflate_batch_array(self.gamma(t), ligand['x'])

        # Concat x and h[categorical]
        ...
        z_t_lig, xh_pocket, eps_t_lig =  \
            self.noised_representation(xh0_lig, xh0_pocket, ligand['mask'], pocket['mask'], gamma_t)
        ...
        if self.vnode_idx is not None:
            # coordinates of virtual atoms should not contribute the error
            squared_error[ligand['one_hot'][:, self.vnode_idx].bool(), :self.n_dims] = 0
        ...
        # cross_entropy term E_q(z0 | x) [lig p(x | z0)].
