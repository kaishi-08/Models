...
class ConditionalDDPMViSNet(nn.Module):
    ...
    def noised_representation(self, xh0_lig, xh0_pocket, lig_mask, pocket_mask, t):
        ...
        print(f"t in noised_representation: min={t.min()}, max={t.max()}")
    ...
    def forward(self, ligand, pocket, return_info=False):
        ...
        lig_mask, pocket_mask = ligand_norm['mask'], pocket_norm['mask']
        t = torch.randint(0, self.T, (ligand['size'].size(0),), device=self.buffer.device) / self.T
        print(f"t after randint: min={t.min()}, max={t.max()}")
        # Kiểm tra bất kỳ thao tác nào với t ở đây
        print(f"t before noised_representation: min={t.min()}, max={t.max()}")
        xh_lig_noisy, xh_pocket_noisy, _, _ = self.noised_representation(xh0_lig, xh0_pocket, lig_mask, pocket_mask, t)
...
class PredefinedNoiseSchedule(nn.Module):
    ...
    def forward(self, t):
        if t.dim() == 0:
            t = t.unsqueeze(0)
        assert t.min() >= 0 and t.max() <= 1 + 1e-5, f"Invalid t: min={t.min()}, max={t.max()}"
