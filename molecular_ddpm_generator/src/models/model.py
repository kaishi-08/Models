...
from ..models.ddpm_visnet import Diffusion
from ..models.vis_dynamics import ViSNetDynamics
from ..utils import molecular_utils


class ConditionalDDPMViSNet(Diffusion):
    """
    Conditional Diffusion Model using ViSNet for ligand generation
    conditioned on protein pocket structure.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        # Ensure dynamics is ViSNet-based
        assert isinstance(self.dynamics, ViSNetDynamics), \
            "ConditionalDDPMViSNet requires ViSNetDynamics"
        
        # Disable pocket coordinate updates by default for conditional model
    ...
    def kl_prior(self, xh_lig, mask_lig, num_nodes):
        ...
        Only for ligand since only ligand is noised.
        """
        ...
        # Compute KL for h-part (ligand atom features)
        ...
        # Compute KL for x-part (ligand coordinates)
    ...
    def log_pxh_given_z0_without_constants(self, ligand, z_0_lig, eps_lig,
        ...
        Pocket is fixed condition, not noised.
        """
        # Discrete properties are predicted directly from z_t.
        z_h_lig = z_0_lig[:, self.n_dims:]

        # Take only part over x (coordinates).
        ...
        # Handle virtual nodes if present
        if hasattr(self, 'vnode_idx') and self.vnode_idx is not None:
        ...
        # Compute delta indicator masks for categorical features.
        # un-normalize
    ...
    def sample_p_xh_given_z0(self, z0_lig, xh0_pocket, lig_mask, pocket_mask,
        ...
        # Sample from normal with COM constraint
        ...
        # Don't modify pocket features for conditional model
    ...
    def sample_normal_zero_com(self, mu_lig, xh0_pocket, sigma, lig_mask,
        ...
        """Samples from a Normal distribution with COM constraint."""
        if fix_noise:
    ...
    def noised_representation(self, xh_lig, xh0_pocket, lig_mask, pocket_mask,
                              gamma_t):
        """Create noised representation for ligand only."""
    ...
    def log_pN(self, N_lig, N_pocket):
        ...
        log p(x,h,N) = log p(x,h|N) + log p(N)
        """
    ...
    def forward(self, ligand, pocket, return_info=False):
        """
        Computes the loss and NLL terms for conditional model.
        Only ligand is noised, pocket serves as fixed condition.
        """
        # Normalize data, take into account volume change in x.
        ligand, pocket = self.normalize(ligand, pocket)

        # Likelihood change due to normalization (only for ligand)
        ...
        # Concatenate x, and h[categorical] for ligand and pocket.
        ...
        # Compute the L2 error for ligand only
        ...
        # Should be close to zero. Only for ligand.
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, xh0_pocket, ligand_mask,
        ...
        # Neural net prediction using ViSNet.
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
                            timesteps=None):
        """
        Draw samples from the generative model given a pocket.
        ...
        lig_mask = molecular_utils.num_nodes_to_batch_mask(
    ...
    def inpaint(self, ligand, pocket, lig_fixed, resamplings=1, return_frames=1,
        ...
        """
        Draw samples from the generative model while fixing parts of the ligand.
        Optionally, return intermediate states for visualization purposes.
        """
        timesteps = self.T if timesteps is None else timesteps
        assert 0 < return_frames <= timesteps
    ...
    def remove_mean_batch(cls, x_lig, x_pocket, lig_indices, pocket_indices):
        """Remove center of mass from ligand, considering both ligand and pocket."""
...
# ------------------------------------------------------------------------------
# Simplified version without subspace trick
# ------------------------------------------------------------------------------
class SimpleConditionalDDPMViSNet(ConditionalDDPMViSNet):
    """
    ...
    def forward(self, ligand, pocket, return_info=False):
        ...
        return super(SimpleConditionalDDPMViSNet, self).forward(
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
        ...
        return super(SimpleConditionalDDPMViSNet, self).sample_given_pocket(
