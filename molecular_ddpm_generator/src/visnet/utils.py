...
class Sphere(nn.Module):
    ...
    def _spherical_harmonics(lmax: int, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        if lmax == 2:
            return torch.stack([sh_1_0, sh_1_1, sh_1_2, sh_2_0, sh_2_1, sh_2_2, sh_2_3, sh_2_4], dim=-1) # 3dim (x,y,z) -> 8_dim: direction features.
...
class Distance(nn.Module):
    ...
    def forward(self, pos, batch):
        edge_index = radius_graph(pos, r=self.cutoff, batch=batch, loop=self.loop, max_num_neighbors=self.max_num_neighbors) #[2, num_edges]
        edge_vec = pos[edge_index[0]] - pos[edge_index[1]] # Direction vector of pair node 
...
class NeighborEmbedding(MessagePassing):
    def __init__(self, hidden_channels, num_rbf, cutoff, input_dim):
        super(NeighborEmbedding, self).__init__(aggr="add")
        
        self.hidden_channels = hidden_channels
        self.input_dim = input_dim
        self.atom_projection  = nn.Linear(input_dim, hidden_channels) #Root: self.embedding = nn.Embedding(z, hidden_Channels)
    ...
    def forward(self, x_onehot, x, edge_index, edge_weight, edge_attr):
        ...
        x_neighbors = self.atom_projection(x_onehot)
...
class OneHotTransform:
    ...
    def __call__(self, data):
        if hasattr(data, 'z'):
            ...
            data.x = F.one_hot(data.z, num_classes=self.max_z).float()
